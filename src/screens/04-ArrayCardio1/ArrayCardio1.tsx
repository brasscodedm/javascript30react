...
export const ArrayCardio1 = () => {
  ...
  const inventorsLiveTogether = inventors.reduce((total, inventor) => {
    return total + (inventor.passed - inventor.year);
  }, 0);

  useEffect(() => {
    console.table(inventorsLiveTogether);
  }, []);

  // 5. Sort the inventors by years lived
  const sortedInventorsByYearsLived = inventors.sort((inv1, inv2) => {
  ...
  const de =
    ...
    links
      ...
      .filter(streetName => streetName && streetName.includes('de'));
  ...
  useEffect(() => {
    console.table(alphabeticallyPeople);
  }, []);
